...
const cardNewPane     = document.getElementById("cardNewPane");
...
function renderPair(index, withTurn) {
  ...
  if (pair.singleCard) {
    cardNewPane.style.display = "none";
    pairView.style.gridTemplateColumns = "minmax(0, 1fr)";
  } else {
    cardNewPane.style.display = "";
    pairView.style.gridTemplateColumns = "repeat(2, minmax(0, 1fr))";
  }
...
function openZoom() {
  ...
  zoomCard2.classList.remove("flipped");
...
function toggleMusic() {
  if (!ytReady) {
    // API not ready yet, queue play intent
    ytPlayPending = true;
    syncMusicBtns(true);
    return;
  }
