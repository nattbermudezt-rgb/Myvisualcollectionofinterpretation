...
const ASSET_VERSION = "20260423";
const CARD_BACK = BASE + "rationalback.png";
const preloadedAssets = Object.create(null);
const resolvedAssets  = Object.create(null); // stores the final resolved src (or null) once a preload finishes
...
function preloadImage(src) {
  if (preloadedAssets[src]) {
    return preloadedAssets[src];
  }

  preloadedAssets[src] = new Promise(function(resolve) {
    var img = new Image();
    img.onload = function() {
      resolvedAssets[src] = src;
      resolve(src);
    };
    img.onerror = function() {
      resolvedAssets[src] = null;
      resolve(null);
    };
    img.src = src;
  });
...
function prefetchPairAssets(index) {
  var pair = pairs[index];
  if (!pair) return;

  preloadImage(assetUrl(pair.back));
  preloadImage(assetUrl(pair.original));

  if (!pair.singleCard) {
    preloadImage(assetUrl(pair.reimagined));
  }
}

function prefetchNearbyPairs(index) {
  [index - 1, index, index + 1, index + 2].forEach(function(pairIndex) {
    prefetchPairAssets(pairIndex);
  });
}
...
function withFallback(imgEl, src, alt) {
  ...
  // If already resolved (preloaded earlier), set synchronously — no placeholder flash
  if (src in resolvedAssets) {
    imgEl.src = resolvedAssets[src] || placeholderSvg;
    return;
  }

  // Not yet loaded — show placeholder while fetching
  imgEl.src = placeholderSvg;

  preloadImage(src).then(function(loadedSrc) {
    if (imgEl.dataset.expectedSrc !== src) return;
    imgEl.src = loadedSrc || placeholderSvg;
  });
}
...
function renderPair(index, withTurn) {
  ...
  prefetchNearbyPairs(index);

  withFallback(cardOriginalImg, assetUrl(pair.original), "Original interpretation");
  withFallback(cardNewImg, assetUrl(pair.reimagined), "New visual interpretation");
...
function openZoom() {
  ...
  withFallback(document.getElementById("zoomBackImg1"), assetUrl(pair.back), "Back " + pair.title);
  withFallback(document.getElementById("zoomBackImg2"), assetUrl(pair.back), "Back " + pair.title);

  // Back face: illustrations (revealed on flip)
  withFallback(zoomOriginal, assetUrl(pair.original), "Original interpretation");
  withFallback(zoomNew, assetUrl(pair.reimagined), "New visual interpretation");
...
function startPresentation() {
  ...
  setTimeout(function() {
    ...
    // Warm up images progressively so pair changes feel immediate during the presentation.
    pairs.forEach(function(_, index) {
      setTimeout(function() {
        prefetchPairAssets(index);
      }, index * 120);
    });
  }, 1300);
}
